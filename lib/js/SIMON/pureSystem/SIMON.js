var SIMON_pureSystem = new function(){
	var Z = [
	...
	this.encrypt = function(message,key){
		...
		var holder = []; var temp = [];
		for(var a = 0; a < T; a++){
			holder = bin_message[0];

			temp = AND(leftShift(bin_message[0],1),leftShift(bin_message[0],8));
			temp = XOR(bin_message[1],temp);
			temp = XOR(temp,leftShift(bin_message[0],2));
			temp = XOR(temp,expandedKey[a]);

			bin_message[0] = temp;
			bin_message[1] = holder;
		}
	...
	this.decrypt = function(message,key){
		...
		var holder = []; var temp = [];
		for(var a = T-1; a >= 0; a--){
			holder = bin_message[1];

			temp = AND(leftShift(bin_message[1],1),leftShift(bin_message[1],8));
			temp = XOR(bin_message[0],temp);
			temp = XOR(temp,leftShift(bin_message[1],2));
			temp = XOR(temp,expandedKey[a]);

			bin_message[1] = temp;
			bin_message[0] = holder;
		}
