var getJS = new function(){
	...
	function gopher(folderAddress,filename,callback){
        ...
        xhttp.onloadend = function(){
            if(this.status == 404){
				console.log('%cgetJS - no _manifest found for "' + folderAddress + '" starting gopher...', "color:rgb(202,136,202); font-style:italic;");
				check(callback,folderAddress,filename);
			}
            else if(this.status == 200){
				console.log('%cgetJS - _manifest found for "' + folderAddress + '"', "color:rgb(202,136,202); font-style:italic;");
				...
				console.log('%cgetJS - latest version: ' + resultArray[resultArray.length-1], "color:rgb(202,136,202); font-style:italic;");
				callback(this.folderAddress+resultArray[resultArray.length-1]+'/'+xhttp.filename);
            }
        }
        xhttp.open('get',folderAddress+'_manifest',true);
        xhttp.send();
