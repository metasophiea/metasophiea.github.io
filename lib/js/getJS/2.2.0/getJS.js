var getJS = new function(){
	...
	function gopher(folderAddress,filename,callback){
        ...
        xhttp.onloadend = function(){
            if(this.status == 404){
				console.log('%c - no manifest found for "' + folderAddress + '" starting gopher...', "color:rgb(202,136,202); font-style:italic;");
				check(callback,folderAddress,filename);
			}
            else if(this.status == 200){
				console.log('%c - manifest found for "' + folderAddress + '"', "color:rgb(202,136,202); font-style:italic;");
	            var responseArray = this.response.split('');
                var resultArray = []; var temp = [];
                for(var a = 0; a < responseArray.length; a++){
                    if(responseArray[a].charCodeAt(0) != 10){ temp.push(responseArray[a]); }
                    else{resultArray.push(temp.join('')); temp = [];}
                }
				if(temp.length != 0){resultArray.push(temp.join('')); }
				console.log('%c - latest version: ' + resultArray[resultArray.length-1], "color:rgb(202,136,202); font-style:italic;");
				callback(this.folderAddress+resultArray[resultArray.length-1]+'/'+xhttp.filename);
            }
        }
				...
				if(this.status == 200){
					...
					if( parseInt(temp_cur[0]) == parseInt(temp_prev[0])+1 &&
						...
						parseInt(temp_cur[2]) == parseInt(temp_prev[2])){
							console.log('%c - gopher search complete', "color:rgb(202,136,202); font-style:italic;");	
							console.log('%c - latest version: ' + manifest[manifest.length-1], "color:rgb(202,136,202); font-style:italic;");										
