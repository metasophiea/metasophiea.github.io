var getJS = new function(){
	...
	this.getManifest = function(URL,callback){
		...
		xhttp.onloadend = function(){
			if(this.status == 200){ callback(processManifest(this.responseText)); }
	...
	function processManifest(rawFile){
		var manifest = rawFile.split(String.fromCharCode(10));
		for(var a = 0; a < manifest.length; a++){console.log( JSON.parse(manifest[a]) );
			if(manifest[a].length == 0){ manifest.splice(a,1); a--; }
			else{
				
//				var temp = manifest[a].split('.'); var temp2 = [];
//				for(var b = 0; b < temp.length-1; b++){temp2.push(temp[b]);}
//				manifest[a] = {'name':temp2.join(' '), 'type':temp[temp.length-1]};
			}
		}
		return manifest;
	}
	function gopher(folderAddress,filename,callback){
		...
		//Check for manifest, run gopher if none is found
        	...
        	xhttp.onloadend = function(){
				...
				var manifest = processManifest(this.responseText); var resultArray = [];
				for(var a = 0; a < manifest.length; a++){
					if(manifest[a].type != 'dir'){
						manifest.splice(a,1);
					}
					else{
						var temp2 = manifest[a].name.split('.'); var correct = true;
						if(temp2.length == 3){
						...
						if(correct){resultArray.push(manifest[a].name.split('.'));}
					}
				}
...
console.log('%cgetJS v2.4.0 loaded', "color:rgb(202,136,202); font-style:italic;");
